            ...
            <?php if($rs_member->username){ ?>
                <form name="frmselect" class="float-right form-group col-lg-6 col-md-6 col-sm-12 " action="his_shop.php" method="get">
                    <select required class="form-control" name="orderID" onchange="document.frmselect.submit();">
                        ...
                        ?>
                            <option value="<?php echo $rsoption['orderID'] ?>" <?php if($_GET['orderID']==$rsoption['orderID']){ echo "selected"; } ?>><?php echo $rsoption['orderID'] ?></option>
                        <?php } ?>
                    </select>
                </form>
                <?php 
                ...
                ?>
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <td class="bg-primary text-white" colspan="4"><center>รหัสใบสั่งซื้อ : <?php echo $rs_order['orderID']." (".$rs_order['dateOrder'].")"; ?>
                                <a href="removeorder.php?orderID=<?php echo $rs_order['orderID']; ?>" onclick="return chkfirm('คุณต้องการลบรายการนี้ใช่หรือไม่')" class="btn btn-danger"><span class="fa fa-trash"></span></a></center></td>
                            </tr>
                            <tr>
                                <th>สินค้า</th>
                                <th>ราคา /1 หน่วย</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="bg-warning text-center " colspan="4">พัสดุ : <a target="_blank" href="http://emsbot.com/#/?s=<?php echo $rs_order['tracking']; ?>"><?php echo $rs_order['tracking']; ?></a></td>
                            </tr>
                        </tbody>
                    </table>
                    <?php 
